...
import { AnchorProvider, Program, BN, BorshCoder } from '@project-serum/anchor';
...
import { ERROR, CURVE_TYPE_ACCOUNTS, PROGRAM_ID, SEEDS, VAULT_PROGRAM_ID, WRAPPED_SOL_MINT, UNLOCK_AMOUNT_BUFFER, } from './constants';
import { StableSwap, TradeDirection } from './curve';
import { ConstantProductSwap } from './curve/constant-product';
import { calculateMaxSwapOutAmount, calculateSwapQuote, computeActualDepositAmount, calculatePoolInfo, getMaxAmountWithSlippage, getMinAmountWithSlippage, getOrCreateATAInstruction, unwrapSOLInstruction, wrapSOLInstruction, getDepegAccounts, } from './utils';
...
const deserializeAccountsBuffer = ([apyPdaBuffer, vaultAReserveBuffer, vaultBReserveBuffer, vaultALpMintBuffer, vaultBLpMintBuffer, poolVaultALpBuffer, poolVaultBLpBuffer, poolLpMintBuffer, clockAccountBuffer,]) => {
    const poolCoder = new BorshCoder(AmmIdl);
...
export default class AmmImpl {
    ...
    static create(connection, pool, tokenInfoA, tokenInfoB, opt) {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const [apyPda] = yield PublicKey.findProgramAddress([Buffer.from(SEEDS.APY), pool.toBuffer()], ammProgram.programId);
            ...
            const poolInfo = calculatePoolInfo(accountsInfo.currentTime, accountsInfo.poolVaultALp, accountsInfo.poolVaultBLp, accountsInfo.vaultALpSupply, accountsInfo.vaultBLpSupply, accountsInfo.poolLpSupply, accountsInfo.apy, swapCurve, vaultA.vaultState, vaultB.vaultState);
    ...
    getUserBalance(owner) {
        return __awaiter(this, void 0, void 0, function* () {
            const account = yield Token.getAssociatedTokenAddress(ASSOCIATED_PROGRAM_ID, TOKEN_PROGRAM_ID, this.poolState.lpMint, owner, true);
            if (!account)
                return new BN(0);
    ...
    getDepositQuote(tokenAInAmount, tokenBInAmount, balance, slippage) {
        invariant(!(!this.isStablePool && !tokenAInAmount.isZero() && !tokenBInAmount.isZero()), 'Constant product only supports balanced deposit');
