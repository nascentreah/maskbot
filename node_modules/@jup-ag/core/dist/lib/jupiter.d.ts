import { AddressLookupTableAccount, BlockhashWithExpiryBlockHeight, Cluster, Connection, FeeCalculator, Keypair, PublicKey, Transaction, TransactionSignature, VersionedTransaction, VersionedTransactionResponse } from '@solana/web3.js';
...
import { getRemoteRouteMap } from '../utils/indexedRouteMap';
...
export { TransactionError, Amm, SwapMode, Owner, getRemoteRouteMap, ammFactory };
export * from './amms';
...
export declare type OnTransaction = (txid: TransactionSignature, totalTxs: number, txDescription: IConfirmationTxDescription, awaiter: Promise<VersionedTransactionResponse | TransactionError | null>) => void;
...
export declare class Jupiter {
     ...
     */
    static load({ connection, cluster, user, platformFeeAndAccounts, quoteMintToReferrer, routeCacheDuration, wrapUnwrapSOL, marketUrl, restrictIntermediateTokens, shouldLoadSerumOpenOrders, ammsToExclude, usePreloadedAddressLookupTableCache, }: JupiterLoadParams): Promise<Jupiter>;
    ...
    static fetchAmms(connection: Connection, cluster: Cluster, ammsToExclude?: SingleLevelAmmValidator, marketUrl?: string, usePreloadedAddressLookupTableCache?: boolean): Promise<{
        amms: Amm[];
...
export declare const executeTransactions: ({ connection, wallet, onTransaction, inputMint, outputMint, sourceAddress, destinationAddress, swapTransaction, owner, wrapUnwrapSOL, }: {
    connection: Connection;
