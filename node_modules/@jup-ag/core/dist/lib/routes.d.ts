...
import { TokenMintAddress, TokenRouteSegments } from './types';
...
export declare type MarketsCache = Array<Omit<AccountInfo<Buffer>, 'data' | 'owner'> & {
    data: [string, 'base64'];
    owner: string;
    pubkey: string;
}>;
export declare type KeyedAccountInfo<T = Buffer> = AccountInfo<T> & {
    pubkey: PublicKey;
    params?: any;
};
...
export declare function getAllAmmsAndSaberWrapperMintsWithALTProvider(connection: Connection, marketsCache: MarketsCache, ammsToExclude: SingleLevelAmmValidator | undefined, usePreloadedAddressLookupTableCache: boolean): Promise<{
    amms: Amm[];
